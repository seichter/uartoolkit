...
int arDetectMarker2New( arToolkit *state, int label_num, int *label_ref,
                                ...
                                int area_max, int area_min, double factor)
{
    ...
    for(i=0; i < label_num; i++ ) {
        ...
        ret = check_square( warea[i], &(state->wmarker_info2[marker_num2]), factor );
        if( ret < 0 ) continue;

        state->wmarker_info2[marker_num2].area   = warea[i];
        state->wmarker_info2[marker_num2].pos[0] = wpos[i*2+0];
        state->wmarker_info2[marker_num2].pos[1] = wpos[i*2+1];
        marker_num2++;
    ...
    for( i=0; i < marker_num2; i++ ) {
        for( j=i+1; j < marker_num2; j++ ) {
            d = (state->wmarker_info2[i].pos[0] - state->wmarker_info2[j].pos[0])
              * (state->wmarker_info2[i].pos[0] - state->wmarker_info2[j].pos[0])
              + (state->wmarker_info2[i].pos[1] - state->wmarker_info2[j].pos[1])
              * (state->wmarker_info2[i].pos[1] - state->wmarker_info2[j].pos[1]);
            if( state->wmarker_info2[i].area > state->wmarker_info2[j].area ) {
                if( d < state->wmarker_info2[i].area / 4 ) {
                    state->wmarker_info2[j].area = 0;
                }
            }
            else {
                if( d < state->wmarker_info2[j].area / 4 ) {
                    state->wmarker_info2[i].area = 0;
                }
            }
        }
    }
    for(i = 0; i < marker_num2; i++ ) {
        ...
        if( state->wmarker_info2[i].area == 0.0 ) {
            for( j=i+1; j < marker_num2; j++ ) {
                state->wmarker_info2[j-1] = state->wmarker_info2[j];
            }
            marker_num2--;
        }
    }
